...
FUSION_MODES = ('add', 'xor', 'or', 'or_clamp')
...
def fuse(x, z, mode):
    ...
    Arithmetic-Boolean (multilinear) forms. For x, z in {0, 1}:
        ...
        or        : x + z -   x*z    -> 0 / 1 / 1 / 1
        or_clamp  : min(1, x + z)    -> clipped ADD; same binary forward as 'or',
                                        different backward.
    Implemented as a plain function (NOT an nn.Module) so that no state_dict key
    is added and every existing checkpoint stays loadable.
    """
    if mode == 'xor':
        return (x + z) - 2 * x * z
    if mode == 'or':
...
class Spikformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        ov_cnt = 0
        if getattr(patch_embed, 'ov_loss', None) is not None:
